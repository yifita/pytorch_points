...
int chamfer_cuda_forward(at::Tensor& xyz1, at::Tensor& xyz2, at::Tensor& dist1, at::Tensor& dist2, at::Tensor& idx1, at::Tensor& idx2){
	...
	AT_DISPATCH_FLOATING_TYPES_AND_HALF(
		xyz1.type(), "NmDistanceKernel", ([&] {
			NmDistanceKernel<scalar_t><<<dim3(batch_size,16,1),BATCH,BATCH*c*sizeof(scalar_t)>>>(batch_size, n, c, xyz1.data<scalar_t>(), m, xyz2.data<scalar_t>(), dist1.data<scalar_t>(), idx1.data<int>());
			NmDistanceKernel<scalar_t><<<dim3(batch_size,16,1),BATCH,BATCH*c*sizeof(scalar_t)>>>(batch_size, m, c, xyz2.data<scalar_t>(), n, xyz1.data<scalar_t>(), dist2.data<scalar_t>(), idx2.data<int>());
			})
	);
...
int labeled_chamfer_cuda_forward(const at::Tensor& xyz1, const at::Tensor& xyz2, const at::Tensor& label1, const at::Tensor& label2,
	...
	AT_DISPATCH_FLOATING_TYPES_AND_HALF(
		xyz1.type(), "NmDistanceKernel", ([&] {
			LabeledNmDistanceKernel<scalar_t><<<dim3(batch_size,16,1),BATCH,BATCH*(c+1)*sizeof(scalar_t)>>>(batch_size, n, c, xyz1.data<scalar_t>(), label1.toType(xyz1.type()).data<scalar_t>(), m,
																								 xyz2.data<scalar_t>(), label2.toType(xyz1.type()).data<scalar_t>(), dist1.data<scalar_t>(), idx1.data<int>());
			LabeledNmDistanceKernel<scalar_t><<<dim3(batch_size,16,1),BATCH,BATCH*(c+1)*sizeof(scalar_t)>>>(batch_size, m, c, xyz2.data<scalar_t>(), label2.toType(xyz1.type()).data<scalar_t>(), n,
																								 xyz1.data<scalar_t>(), label1.toType(xyz1.type()).data<scalar_t>(), dist2.data<scalar_t>(), idx2.data<int>());
			})
	);
